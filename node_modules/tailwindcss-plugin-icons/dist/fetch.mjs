...
import { u as uriToFilename, T as TailwindcssPluginIconsError } from './internal/shared.mjs';
import 'crypto';
...
async function makeRequest(uri) {
  ...
  const protocol = await (uri.startsWith("https") ? import('https') : import('http'));
  return new Promise((resolve, reject) => {
    protocol.get(uri, (response) => {
      const writeStream = createWriteStream(filePath).on("finish", resolve);
      response.on("data", writeStream.write.bind(writeStream)).on("end", async () => {
